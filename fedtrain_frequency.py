...
def pretrain(dataset, **kwargs):
    ...
    config_A = Config(model=RPQRec, dataset='O', config_file_list=props, config_dict=kwargs)
    config_B = Config(model=RPQRec, dataset='A', config_file_list=props, config_dict=kwargs)
    ...
    item_pq_A = rpq_codes[:spilt_point + 1]
    item_pq_B = torch.cat([rpq_codes[0].unsqueeze(0), rpq_codes[spilt_point + 1:]], dim=0)
    pretrain_dataset_A.rpq_codes = item_pq_A
    pretrain_dataset_B.rpq_codes = item_pq_B
