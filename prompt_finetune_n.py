...
from model.full_prompt_n import id_prompt
from model.light_prompt_n import seq_prompt
from model.item_prompt_n import item_prompt
...
def finetune(model_name, dataset, pretrained_file='', finetune_mode='', **kwargs):
    ...
    prompt_model = seq_prompt(config_A, pretrain_data_A.dataset, model_A).to(config['device'])
    prompt_model.rpq_codes.to(config['device'])
    
    for param in prompt_model.rpqrec.parameters():
        param.requires_grad = False
    for param in prompt_model.rpqrec.pq_code_embedding.parameters():
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', type=str, default='MS', help='dataset name')
    parser.add_argument('-p', type=str, default='save_MS/RPQRec-Aug-15-2024_01-09-33.pth', help='pre-trained model path')
