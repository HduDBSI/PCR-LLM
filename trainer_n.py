...
class RPQRecTrainer(Trainer):
    def __init__(self, config, model):
        super(RPQRecTrainer, self).__init__(config, model)
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if epoch_idx < self.reassign_steps:
                ...
                model.index_assignment_flag = True
                model.trans_matrix.requires_grad = True
                ...
                model.trans_matrix.requires_grad = False
                model.reassigned_code_embedding = model.code_projection()
                embed_weight = model.reassigned_code_embedding.detach()
                embed_weight[0, :] = 0
                reassigned_state_dict = OrderedDict()
                reassigned_state_dict['weight'] = embed_weight
                model.rpq_code_embedding.load_state_dict(reassigned_state_dict)
                model.index_assignment_flag = False
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        loss_func = loss_func or model.calculate_loss
        total_loss = None
        iter_data = (
            tqdm(
                train_data,
        ...
        for batch_idx, interaction in enumerate(iter_data):
            # Index Assignment Special
            if model.index_assignment_flag:
                model.reassigned_code_embedding = model.code_projection()
            # End Index Assignment
