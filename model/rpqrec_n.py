...
class RPQRec(SequentialRecommender):
    ...
    def seq_item_contrastive_task(self, seq_output, same_pos_id, interaction):
        ...
        neg_logits = torch.matmul(seq_output, pos_items_emb.transpose(0,
                                                                      1)) / self.temperature  # 乘转置后的得到2048x2048的，就是相当于每一条表示对每一个target的预测
        neg_logits = torch.where(same_pos_id, torch.tensor([0], dtype=torch.float, device=same_pos_id.device),
                                 neg_logits)  # 这行意思是为same_pos_id里面值为true时用的是[0]这个张量，是false时用的是neg_logits
    ...
    def pretrain(self, interaction):
        ...
        same_pos_id = torch.logical_xor(same_pos_id, torch.eye(pos_id.shape[0], dtype=torch.bool,
