...
class id_prompt(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        position_embedding = self.rpqrec.position_embedding(position_ids)
        item_emb = self.item_embedding(item_seq)
        item_emb = item_emb + position_embedding
        item_emb = self.LayerNorm(item_emb)
        item_trm_input = self.dropout(item_emb)
        extended_attention_mask = self.rpqrec.get_attention_mask(item_seq)
        ...
        feature_output = self.rpqrec.forward(item_seq, item_seq_len)
    ...
    def get_prompt(self, seq_out):
        ...
        prompt = self.prompt_dropout(prompt)#3.27新增
        prompt, _ = self.attn_layer(seq_out, prompt, prompt)
        return prompt

    def calculate_item_emb(self):
        rpq_code_emb = self.rpqrec.rpq_code_embedding(self.rpq_codes).mean(dim=-2)
        return rpq_code_emb
    ...
    def calculate_loss(self, interaction):
        ...
        base_loss = self.loss_fct(logits,pos_items)

        #L2 3.27
        l2_lambda = 1e-4
        prompt_l2_loss=l2_lambda* torch.norm(self.prompts,p=2)

        loss = base_loss+prompt_l2_loss
