...
class id_prompt(SequentialRecommender):
    def __init__(self, config, dataset, rpqrec):
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        # 使用 DataParallel 时通过 module 访问模型的属性和方法
        position_embedding = self._get_rpqrec_attr('position_embedding')(position_ids)
        item_emb = self.item_embedding(item_seq)
        item_emb = item_emb + position_embedding
        item_emb = self.LayerNorm(item_emb)
        item_trm_input = self.dropout(item_emb)
        extended_attention_mask = self._get_rpqrec_method('get_attention_mask')(item_seq)
        ...
        feature_output = self._get_rpqrec_method('forward')(item_seq, item_seq_len)
        prompt = self.get_prompt(seq_out=feature_output.unsqueeze(1)).squeeze(1)
        output_concat = torch.cat((prompt, item_output, feature_output), -1)  # [B 3*H]
        output = self.concat_layer(output_concat)
        output = self.LayerNorm(output)
        seq_output = self.dropout(output)
        return seq_output

    def _get_rpqrec_attr(self, attr_name):
        """获取 rpqrec 模型中的属性，支持 DataParallel"""
        if isinstance(self.rpqrec, nn.DataParallel):
            return getattr(self.rpqrec.module, attr_name)
        return getattr(self.rpqrec, attr_name)

    def _get_rpqrec_method(self, method_name):
        """获取 rpqrec 模型中的方法，支持 DataParallel"""
        if isinstance(self.rpqrec, nn.DataParallel):
            return getattr(self.rpqrec.module, method_name)
        return getattr(self.rpqrec, method_name)
    ...
    def calculate_item_emb(self):
        pq_code_emb = self._get_rpqrec_attr('pq_code_embedding')(self.pq_codes).mean(dim=-2)
        return pq_code_emb
