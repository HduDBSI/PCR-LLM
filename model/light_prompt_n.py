...
class seq_prompt(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len):
        # 使用 DataParallel 时通过 module 访问模型的 forward 方法
        if isinstance(self.rpqrec, nn.DataParallel):
            return self.rpqrec.module.forward(item_seq, item_seq_len)
        else:
            return self.rpqrec.forward(item_seq, item_seq_len)

    def calculate_item_emb(self):
        # 使用 DataParallel 时通过 module 访问模型的 calculate_item_emb 方法
        if isinstance(self.rpqrec, nn.DataParallel):
            pq_code_emb = self.rpqrec.module.pq_code_embedding(self.pq_codes).mean(dim=-2)
        else:
            pq_code_emb = self.rpqrec.pq_code_embedding(self.pq_codes).mean(dim=-2)
        return pq_code_emb
