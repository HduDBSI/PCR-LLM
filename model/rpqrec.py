...
class RPQRec(SequentialRecommender):
    def __init__(self, config, dataset):
        ...
        self.rpq_codes = dataset.rpq_codes
        ...
        self.rpq_code_embedding = nn.Embedding(
    ...
    def code_projection(self):
        ...
        raw_embed = self.rpq_code_embedding.weight.reshape(self.code_dim, self.code_cap + 1, -1)
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        rpq_code_seq = self.rpq_codes[item_seq]
        if self.index_assignment_flag:
            rpq_code_emb = F.embedding(rpq_code_seq, self.reassigned_code_embedding, padding_idx=0).mean(dim=-2)
        else:
            rpq_code_emb = self.rpq_code_embedding(rpq_code_seq).mean(dim=-2)
        input_emb = rpq_code_emb + position_embedding
    ...
    def calculate_item_emb(self):
        if self.index_assignment_flag:
            rpq_code_emb = F.embedding(self.rpq_codes, self.reassigned_code_embedding, padding_idx=0).mean(dim=-2)
        else:
            rpq_code_emb = self.rpq_code_embedding(self.rpq_codes).mean(dim=-2)  # self.rpq_codes
        return rpq_code_emb  # [B H]
    ...
    def generate_fake_neg_item_emb(self, item_index):
        ...
        return self.rpq_code_embedding(fake_item_idx).mean(dim=-2)

    def seq_item_contrastive_task(self, seq_output, same_pos_id, interaction):
        pos_id = interaction['item_id']
        pos_rpq_code = self.rpq_codes[pos_id]
        if self.index_assignment_flag:
            pos_items_emb = F.embedding(pos_rpq_code, self.reassigned_code_embedding, padding_idx=0).mean(dim=-2)
        else:
            pos_items_emb = self.rpq_code_embedding(pos_rpq_code).mean(dim=-2)
        ...
        fake_item_emb = self.generate_fake_neg_item_emb(pos_rpq_code)
    ...
    def fedtrain(self, interaction):
        ...
        pos_id = interaction['item_id']
        pos_rpq_code = self.rpq_codes[pos_id]
        neg_items_emb = self.rpq_code_embedding(self.neg_code).mean(dim=-2)
