...
def pretrain(dataset, **kwargs):
    ...
    config_A = Config(model=RPQRec, dataset='B', config_file_list=props, config_dict=kwargs)
    config_B = Config(model=RPQRec, dataset='M', config_file_list=props, config_dict=kwargs)
    ...
    global_embedding = nn.Embedding(
        config['code_dim'] * (1 + config['code_cap']), config['hidden_size'], padding_idx=0
    ).to(config['device'])
    global_embedding.weight.data.normal_(mean=0.0, std=config['initializer_range'])

    if isinstance(model_A, nn.DataParallel):
        model_A.module.pq_code_embedding.load_state_dict(global_embedding.state_dict())
        model_B.module.pq_code_embedding.load_state_dict(global_embedding.state_dict())
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', type=str, default='BM', help='dataset name')
