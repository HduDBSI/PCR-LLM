...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch_A(self, train_data, epoch_idx, global_embedding, loss_func=None, show_progress=False):
        ...
        self.model_A.train()
        embedding_grad_sum = torch.zeros_like(self.model_A.rpq_code_embedding.weight)
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            losses = self.model_A.calculate_loss(interaction) #+ 0.005 * torch.norm(self.model_A.rpq_code_embedding.weight - global_embedding.weight) ** 2
            ...
            embedding_grad_sum += self.model_A.rpq_code_embedding.weight.grad.clone()
    ...
    def _train_epoch_B(self, train_data, epoch_idx, global_embedding, loss_func=None, show_progress=False):
        self.model_B.train()
        embedding_grad_sum = torch.zeros_like(self.model_B.rpq_code_embedding.weight)
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            losses = self.model_B.calculate_loss(interaction) #+ 0.005 * torch.norm(self.model_B.rpq_code_embedding.weight - global_embedding.weight) ** 2
            if isinstance(losses, tuple):
                loss = sum(losses)
                loss_tuple = tuple(per_loss.item() for per_loss in losses)
                total_loss = loss_tuple if total_loss is None else tuple(map(sum, zip(total_loss, loss_tuple)))
            ...
            embedding_grad_sum += self.model_B.rpq_code_embedding.weight.grad.clone()
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('Loss: ' + str(losses.item()), 'yellow'))
        return total_loss, embedding_grad_sum
...
class FedtrainTrainer(Trainer):
    ...
    def __init__(self, config_A, config_B, model_A, model_B, global_embedding):
        ...
        self.epsilon = self.config_A['epsilon'] 
        self.delta = 1e-5 
        self.num_bits = 8  
        self.num_buckets = 2 ** self.num_bits 
        self.clip_threshold = 0.12  
        self.scale_factor = self.clip_threshold / self.num_buckets  
        self.prob_p = (np.exp(self.epsilon) + 1) / (np.exp(self.epsilon) + 2)
        self.prob_q = 1 / (np.exp(self.epsilon) + 2) 

        self.metric_alpha = self.config_A.get('metric_alpha', 0.7) 
        self.temp_T = self.config_A.get('metric_temp', 0.1)  
        self.ema_beta = self.config_A.get('metric_ema_beta', 0.6)  
        self.min_weight = self.config_A.get('metric_min_weight', 0.05)  
        self.mix_with_size = self.config_A.get('mix_with_size', 0.3)  
        self.prev_weights = None   
        # self.attn_layer = nn.MultiheadAttention(config_A['hidden_size'], num_heads=4).to(config_A['device'])

    @torch.no_grad()
    def _validate_client_A(self):
        result = self._valid_epoch_A(self.valid_data_A) 
        auc = float(result.get('auc', 0.5))
        recall = float(result.get('recall', 0.0))
        return {'auc': auc, 'recall': recall}

    @torch.no_grad()
    def _validate_client_B(self):
        result = self._valid_epoch_B(self.valid_data_B)
        auc = float(result.get('auc', 0.5))
        recall = float(result.get('recall', 0.0))
        return {'auc': auc, 'recall': recall}

    def _metrics_to_weights(self, metrics_A, metrics_B, size_weights=None, device=None):

        device = device or self.device

        score_A = self.metric_alpha * metrics_A['auc'] + (1 - self.metric_alpha) * metrics_A['recall']
        score_B = self.metric_alpha * metrics_B['auc'] + (1 - self.metric_alpha) * metrics_B['recall']
        scores = torch.tensor([score_A, score_B], dtype=torch.float32, device=device)

        perf_w = torch.softmax(scores / max(self.temp_T, 1e-6), dim=0)

        if size_weights is not None:
            size_weights = size_weights.to(device)
            size_weights = size_weights / (size_weights.sum() + 1e-12)
            w = self.mix_with_size * size_weights + (1 - self.mix_with_size) * perf_w
        else:
            w = perf_w
        ...
        if self.prev_weights is None:
            self.prev_weights = w.detach()
        else:
            w = self.ema_beta * self.prev_weights + (1 - self.ema_beta) * w
            w = w / (w.sum() + 1e-12)
            self.prev_weights = w.detach()
    ...
    def randomize(self, quantized_gradient):
        ...
        randomized_gradient = (quantized_gradient + coin_flips * noise) % self.num_buckets 
    ...
    def fedtrain(self, train_data_A, train_data_B, weight, verbose=True, show_progress=False):
        for epoch_idx in range(self.start_epoch, self.pretrain_epochs):
            ...
            try:
                metrics_A = self._validate_client_A()
                metrics_B = self._validate_client_B()
            except Exception as e:
                if verbose:
                    self.logger.warning(f'Validation failed or not provided, fallback to size weights. Err: {e}')
                dynamic_weights = weight.to(self.device)
                dynamic_weights = dynamic_weights / (dynamic_weights.sum() + 1e-12)
            else:
                dynamic_weights = self._metrics_to_weights(metrics_A, metrics_B, size_weights=weight,
                                                           device=self.device)

            global_embedding_params = self.global_embedding.state_dict()
            agg_grad = self.aggregate(client_gradients, dynamic_weights)
            for key in global_embedding_params.keys():
                global_embedding_params[key] = global_embedding_params[key] - \
                                               self.learning_rate * agg_grad
            self.global_embedding.load_state_dict(global_embedding_params)
            self.model_A.rpq_code_embedding.load_state_dict(self.global_embedding.state_dict())
            self.model_B.rpq_code_embedding.load_state_dict(self.global_embedding.state_dict())
    ...
    def _valid_epoch_B(self, valid_data_B):
        pass

    def _valid_epoch_A(self, valid_data_A):
        pass